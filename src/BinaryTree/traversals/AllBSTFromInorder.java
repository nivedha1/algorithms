...
import java.util.Vector;
    ...
    class AllBSTFromInorder {
        ...
        // vector of trees. 
        Vector<TNode> getTrees(int arr[], int start, int end) {

            // List to store all possible trees 
            Vector<TNode> trees = new Vector<TNode>(); 
        ...
        /* Iterating through all values from start to end 
         ...
         recursively */
            for (int i = start; i <= end; i++) {
                ...
                Vector<TNode> ltrees = getTrees(arr, start, i - 1);

                /* Constructing right subtree */
                Vector<TNode> rtrees = getTrees(arr, i + 1, end); 
        ...
        public static void main(String args[]) {
            int in[] = {4, 5, 7};
            int n = in.length;
            AllBSTFromInorder tree = new AllBSTFromInorder();
            Vector<TNode> trees = tree.getTrees(in, 0, n - 1);
